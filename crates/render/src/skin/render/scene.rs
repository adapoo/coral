...
use crate::skin::model::{BodyPart, Pose};
use crate::skin::output::{OutputType, RenderOutput};
...
impl Renderer {
    ...
    fn render_3d(
        ...
        let image = RgbaImage::from_raw(width, height, image_data)
            .ok_or_else(|| SkinError::Render("failed to create output image".into()))?;

        Ok(RenderOutput { image })
    }
    ...
    fn render_face(&self, skin: &Skin, size: u32) -> RenderOutput {
        ...
        for y in 0..size {
            for x in 0..size {
                let src_x = 8 + (x * 8 / size);
                let src_y = 8 + (y * 8 / size);
                let base_pixel = texture.get_pixel(src_x, src_y);

                let overlay_x = 40 + (x * 8 / size);
                let overlay_y = 8 + (y * 8 / size);
                let overlay_pixel = texture.get_pixel(overlay_x, overlay_y);

                let pixel = blend_pixels(*base_pixel, *overlay_pixel);
...
fn blend_pixels(base: Rgba<u8>, overlay: Rgba<u8>) -> Rgba<u8> {
    ...
    match alpha {
        ...
        a if a == 1.0 => overlay,
        _ => {
            let blend = |b: u8, o: u8| ((b as f32 * (1.0 - alpha)) + (o as f32 * alpha)) as u8;
            Rgba([
                blend(base.0[0], overlay.0[0]),
                blend(base.0[1], overlay.0[1]),
                blend(base.0[2], overlay.0[2]),
                255,
            ])
        }
    }
