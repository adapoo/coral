...
use crate::skin::{Skin, model::{BodyPart, CubeFaceUvs, FaceUv, Pose}};
...
impl Mesh {
    ...
    pub fn add_cube(
        ...
        uvs: &CubeFaceUvs, transform: Mat4, expansion: f32,
    ) {
        ...
        self.add_face(transform, [Vec3::new(x0, y1, z0), Vec3::new(x1, y1, z0), Vec3::new(x0, y0, z0), Vec3::new(x1, y0, z0)], &uvs.front);
        self.add_face(transform, [Vec3::new(x1, y1, z1), Vec3::new(x0, y1, z1), Vec3::new(x1, y0, z1), Vec3::new(x0, y0, z1)], &uvs.back);
        self.add_face(transform, [Vec3::new(x1, y1, z0), Vec3::new(x1, y1, z1), Vec3::new(x1, y0, z0), Vec3::new(x1, y0, z1)], &uvs.left);
        self.add_face(transform, [Vec3::new(x0, y1, z1), Vec3::new(x0, y1, z0), Vec3::new(x0, y0, z1), Vec3::new(x0, y0, z0)], &uvs.right);
        self.add_face(transform, [Vec3::new(x1, y1, z1), Vec3::new(x0, y1, z1), Vec3::new(x1, y1, z0), Vec3::new(x0, y1, z0)], &uvs.top);
        self.add_face(transform, [Vec3::new(x0, y0, z1), Vec3::new(x1, y0, z1), Vec3::new(x0, y0, z0), Vec3::new(x1, y0, z0)], &uvs.bottom);
    }
    ...
    fn add_face(&mut self, transform: Mat4, corners: [Vec3; 4], uv: &FaceUv) {
        ...
        self.vertices.extend([
            Vertex { position: p0.to_array(), uv: uv.top_left.to_array(), normal },
            Vertex { position: p1.to_array(), uv: uv.top_right.to_array(), normal },
            Vertex { position: p2.to_array(), uv: uv.bottom_left.to_array(), normal },
            Vertex { position: p3.to_array(), uv: uv.bottom_right.to_array(), normal },
        ]);
    ...
    pub fn build_from_skin(skin: &Skin, pose: &Pose, parts: &[BodyPart]) -> Self {
        ...
        for &part in parts {
            ...
            mesh.add_cube(part.position(slim), part.size(slim), &part.uvs(slim), transform, part.layer_expansion());
        }
        mesh
    }
