...
use crate::canvas::{color::{BOX_BACKGROUND, blend}, context::DrawContext, shape::Shape, text::TextRenderer};
...
impl TextBox {
    ...
    pub fn push(mut self, line: MCText) -> Self { self.lines.push(line); self }
    pub fn scale(mut self, scale: f32) -> Self { self.scale = scale; self }
    pub fn line_spacing(mut self, spacing: f32) -> Self { self.line_spacing = spacing; self }
    pub fn align_x(mut self, align: Align) -> Self { self.align_x = align; self }
    pub fn align_y(mut self, align: Align) -> Self { self.align_y = align; self }
    pub fn shadow(mut self, shadow: bool) -> Self { self.shadow = shadow; self }
    pub fn padding(mut self, x: u32, y: u32) -> Self { self.padding = (x, y); self }
    pub fn corner_radius(mut self, radius: u32) -> Self { self.corner_radius = radius; self }
    pub fn background(mut self, color: Rgba<u8>) -> Self { self.background = color; self }
    pub fn width(mut self, width: u32) -> Self { self.width = Some(width); self }
    pub fn height(mut self, height: u32) -> Self { self.height = Some(height); self }

    fn measure_lines(&self, renderer: &TextRenderer) -> Vec<(f32, f32)> {
        let font_size = self.scale * 16.0;
        self.lines.iter().map(|line| renderer.measure(line, font_size)).collect()
    }
...
impl Shape for TextBox {
    fn measure(&self, renderer: &TextRenderer) -> (u32, u32) {
        ...
        (
            self.width.unwrap_or(content_w.ceil() as u32 + self.padding.0 * 2),
            self.height.unwrap_or(content_h.ceil() as u32 + self.padding.1 * 2),
        )
    }
    ...
    fn draw(&self, ctx: &mut DrawContext) {
        ...
        let box_w = self.width.unwrap_or(content_w.ceil() as u32 + self.padding.0 * 2);
        let box_h = self.height.unwrap_or(content_h.ceil() as u32 + self.padding.1 * 2);
        ...
        for py in 0..box_h {
            for px in 0..box_w {
                ...
                if is_outside_rounded_rect(px, py, box_w, box_h, radius) { continue; }
            ...
            self.lines.iter().zip(measurements.iter()).enumerate()
        {
            ...
            ctx.renderer.draw(
                ...
                line, font_size, self.shadow,
            );
            cursor_y += line_h;
            if i < self.lines.len() - 1 { cursor_y += gap; }
        }
    }
