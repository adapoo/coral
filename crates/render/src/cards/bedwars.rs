...
pub fn render_bedwars(
    ...
    let canvas = canvas
        .draw(
            ...
            &WinstreaksBox {
                winstreaks,
                current_ws: mode_stats.winstreak,
            },
        )
...
impl<'a> ModeShareBox<'a> {
    fn new(mode: &'a ModeStats, overall: &'a ModeStats) -> Self {
        Self { mode, overall }
    }
    ...
    fn pct(mode_val: u64, overall_val: u64) -> f64 {
        if overall_val == 0 {
            ...
            mode_val as f64 / overall_val as f64 * 100.0
        }
    }
}
...
fn slumber_box(slumber: &SlumberInfo) -> TextBox {
    let tickets_text = MCText::new()
        .span("Tickets: ")
        .color(NamedColor::Gray)
        .then(&format_number(slumber.tickets))
    ...
    TextBox::new()
        ...
        .push(
            MCText::new()
                .span("Lifetime: ")
                .color(NamedColor::Gray)
                .then(&format_number(slumber.total_tickets_earned))
        ...
        .push(
            MCText::new()
                .span("XP Doublers: ")
                .color(NamedColor::Gray)
                .then(&format_number(slumber.doublers))
                .color(NamedColor::Green)
                .build(),
        )
}
...
impl Shape for WinstreaksBox<'_> {
    fn draw(&self, ctx: &mut DrawContext) {
        ...
        let mut lc = ctx.at(padding as i32, y as i32);
        TextBlock::new()
            .push(current_line)
            .scale(scale)
            .draw(&mut lc);
        ...
        for (i, streak) in self.winstreaks.streaks[..display_count].iter().enumerate() {
            ...
            let icon_y = y + (line_h - icon_size as f32) / 2.0;
            let mut icon_ctx = ctx.at(padding as i32, icon_y as i32);
            ...
            let mut left_ctx = ctx.at(text_x as i32, y as i32);
            TextBlock::new().push(left).scale(scale).draw(&mut left_ctx);

            let (rw, _) = ctx.renderer.measure(&right, font);
            let rx = padding as f32 + inner_w as f32 - rw;
            let mut right_ctx = ctx.at(rx as i32, y as i32);
            TextBlock::new()
                .push(right)
                .scale(scale)
                .draw(&mut right_ctx);
...
impl Shape for SkinSection<'_> {
    fn draw(&self, ctx: &mut DrawContext) {
        ...
        if let Some(skin) = &self.skin {
            ...
            let mode_top = mode_y;
            let available_h = mode_top - level_bottom;
