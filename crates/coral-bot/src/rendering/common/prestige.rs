...
pub fn prestige_colors(level: u32) -> Vec<NamedColor> {
    ...
    match level {
        ...
        1900..=1999 => vec![
            Gray, DarkPurple, DarkPurple, DarkPurple, DarkPurple, DarkGray, Gray,
        ],
        ...
        4300..=4399 => vec![
            Black, DarkPurple, DarkGray, DarkGray, DarkPurple, DarkPurple, Black,
        ],
        4400..=4499 => vec![
            DarkGreen,
            DarkGreen,
...
pub fn build_prestige_text(text: &str, colors: &[NamedColor]) -> MCText {
    let mut result = MCText::default();
    for (i, ch) in text.chars().enumerate() {
        let color = colors.get(i).copied().unwrap_or(NamedColor::White);
        result += MCText::new().span(&ch.to_string()).color(color).build();
    }
    result
}
