use serenity::all::{
    ChannelId, Context, CreateAttachment, CreateComponent, CreateContainer,
    ...
    CreateUnfurledMediaItem, MessageFlags, MessageId, UserId,
};
...
pub async fn post_tag_removed(
    ...
    send_to_mod_channel(ctx, data, container, files).await;
}
...
pub async fn post_tag_changed(
    ...
    let old_tag_display = CreateTextDisplay::new(format!(
        "Previous: {} {}\n> {}\n{}",
        old_emote,
        old_display,
    ...
    let new_tag_display = CreateTextDisplay::new(format!(
        "New: {} {}\n> {}\n{}",
        new_emote,
        new_display,
        sanitize_reason(&new_tag.reason),
        new_added_line
    ));

    let username = get_username(ctx, changed_by).await;
    let action_display = CreateTextDisplay::new(format!("-# {} by `@{}`", title, username));
    let uuid_line = CreateTextDisplay::new(format!("-# UUID: {}", dashed_uuid));

    let container = CreateContainer::new(vec![
        CreateContainerComponent::Section(header),
        CreateContainerComponent::TextDisplay(old_tag_display),
        CreateContainerComponent::Separator(CreateSeparator::new(true)),
        CreateContainerComponent::TextDisplay(new_tag_display),
        CreateContainerComponent::TextDisplay(action_display),
        CreateContainerComponent::TextDisplay(uuid_line),
        CreateContainerComponent::Separator(CreateSeparator::new(true)),
    ])
...
async fn post_to_blacklist_channel(
    ...
    let files = face.into_iter().collect();
    send_container(ctx, channel_id, container, files).await
}
