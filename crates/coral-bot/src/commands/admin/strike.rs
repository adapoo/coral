...
async fn handle_add(
    ...
    let sub_options = command.data.options.first()
    ...
    let target_id = sub_options.iter()
        .find(|o| o.name == "user")
        .and_then(|o| match &o.value {
    ...
    send_reply(ctx, command, &format!("Strike added for <@{target_id}>: \"{reason}\""), false).await
}
